...
export default function Gallery({ data, theme }: Props) {
  ...
  // 갤러리 사진첩 모달이 열렸을 때 백그라운드 이미지 프리로드
  useEffect(() => {
  ...
  // 사진첩 모달이 열렸을 때 배경(메인 페이지) 스크롤 방지
  useEffect(() => {
    if (albumOpen) {
  ...
  const selectPhoto = (targetIdx: number) => {
    if (targetIdx !== index) {
          ...
          <AnimatePresence>
            {albumOpen && hasPhotos && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[99990] flex items-center justify-center bg-black/85 backdrop-blur-md p-3 sm:p-4"
                onClick={(e) => {
